...
int validate_args(const cli_args_t *args) {
    ...
    if (args->short_output) output_formats++;
    if (args->json_output) output_formats++;
    if (args->show_tree) output_formats++;
    if (args->show_env) output_formats++;

    if (output_formats > 1) {
        print_error("Cannot specify multiple output formats (--short, --json, --tree, --env)");
        return -1;
    }
